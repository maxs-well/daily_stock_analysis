...
### 2.1 Deep Research / EventMonitor 存在“文档和配置仍在，但实现缺失”的能力漂移
...
### 2.3 Agent 历史并非“无限增长”，但缺少 token-aware 治理
...
### 2.4 策略回测存在“看似支持策略，实际不是策略级统计”的语义问题
...
该问题成立，建议在进入 `try` 前先给 `stock_name` 一个安全默认值，例如 `code`。

### 3.2 `slice_at_max_bytes()` 的返回类型签名错误

`src/formatters.py` 中 `slice_at_max_bytes()` 的签名标注为返回 `str`，但实际返回 `(truncated, remaining)` 二元组。

这是确定性的类型签名错误，会影响静态检查和调用者理解。

### 3.3 单 Agent / 工具并发的超时治理不完整
...
> Agent 超时治理并非完全缺失，但 single-agent / tool fan-out 路径仍不完整。

### 3.4 CORS 存在 `* + credentials` 冲突

`api/app.py` 中当 `CORS_ALLOW_ALL=true` 时，会将 `allow_origins=["*"]`，同时仍设置 `allow_credentials=True`。

这在浏览器 credential 模式下是不兼容的，属于明确的配置级错误风险。

### 3.5 配置解析存在直接 `int()` / `float()` 转换
